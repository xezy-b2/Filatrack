...
export default async function NewSpoolPage({
  ...
  // Pré-remplissage optionnel depuis la suggestion "bobine détectée" de
  // /dashboard/printer (matière/couleur lues via la puce RFID de l'AMS).
  ...
  const detected = !!(material || colorHex);
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-6">
        <SpoolForm action={createSpool} submitLabel="Ajouter la bobine" initialData={{ material, colorHex }} />
      </div>
    </div>
  );
}
